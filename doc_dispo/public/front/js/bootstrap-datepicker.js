	...
	Datepicker.prototype = {
      ...
      var localDate = this._utc_to_local(this.viewDate);
		...
		click: function(e){
			...
			if (!target.hasClass('disabled')){
				...
				if (target.hasClass('day')){
					...
					let mois = (month < 10) ? '0'+(month+1) : month+1;
					let jour = (day < 10) ? '0'+day : day;

					dayValpiere = year+'-'+mois+'-'+jour;
					/*<li>
                        <input type="radio" id="radio1" name="radio_time" value="{{ $creneau->heure_creneau }}">
                        <label for="radio1">{{ $creneau->heure_creneau }}</label>
                    </li>*/

					listeUL1.textContent = ''
					...
					for(let i = 0; i < creneaux.length; i++)
					{
						...
						if(creneaux[i].date_creneau === dayValpiere)
						{
							...
							input.setAttribute('name', 'radio_time')
							input.setAttribute('value', creneaux[i].id)


							let label = document.createElement('label')
							label.setAttribute('for', 'radio'+i)
							label.textContent = creneaux[i].heure_creneau
							...
							if(parseFloat(creneaux[i].heure_creneau) <= 12.00)
							{
								listeUL1.appendChild(li)
							}
							else{
								listeUL2.appendChild(li)
							}

							creneau.value = creneaux[i].id;
		...
		dateIsDisabled: function(date){
			for(let i = 0; i < creneaux.length; i++)
			{
				if(new Date(creneaux[i].date_creneau).getTime() === date.getTime())
				{
					return false
				}
